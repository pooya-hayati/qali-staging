...
- **Indexing threshold**: `Shop::CHAIN_NOINDEX_MIN_PRODUCTS = 8` (flagging for confirmation, not silently final) — chain pages under 8 matching products get `noindex,follow` via `wpseo_robots_array` (still renders normally, still linkable/crawlable, just not indexed); 8+ gets whatever Yoast/robots would otherwise say. Chosen as a low floor given this site's category sizes range roughly 5–300+ products per PROGRESS-LOG's own pagination data — thin enough to filter out near-empty combinations without being so high it noindexes every real 2-attribute intersection. **Could not be visually confirmed live**: `blog_public` is `0` on this site (Settings → Reading → "discourage search engines"), so `wp_robots_no_robots()` forces `noindex,nofollow` site-wide on *every* page right now, including the homepage — confirmed this is the site-wide staging setting, not a bug in this new code, by checking the homepage and shop page show the identical tag. The per-chain logic is verified correct by code path (it runs, and would take effect the moment that staging setting is off), just not observable in isolation until then.
...
**Flagging for explicit confirmation, not silently finalized:** the `8`-product noindex floor, and the alphabetical-order canonical URL (an addition beyond the literal spec, to prevent N! duplicate-content variants of the same chain).
